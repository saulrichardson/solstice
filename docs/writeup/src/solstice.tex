...
All text-based evidence extraction and verification uses gpt-4.1 with temperature=0. This model was chosen specifically for its needle-in-haystack capabilities:
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=0pt]
...
\item \textbf{Contextual interpretation}: Goes beyond OCR to understand what visual evidence actually demonstrates
\end{itemize}
...
\item \textbf{Prompt engineering}: Extraction tasks benefit from zero-temperature, deterministic responses. Visual reasoning tasks need more flexible interpretation capabilities
