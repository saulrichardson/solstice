...
\item \textbf{Gateway Pattern}: All LLM calls go through an HTTP gateway service, which handles request routing and provider abstraction.
...
\item Parallel processing is limited with a semaphore (max 5 concurrent calls) to avoid overwhelming the API
\end{itemize}
