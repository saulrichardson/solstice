...
Solstice keeps \\textit{every} intermediate and final result on disk in human-readable form. This makes it easy to
inspect the system, debug a stage, or build visualisations without rerunning heavy models.
...
If you want the raw, per-agent outputs (e.g. to analyse the verifier’s chain-of-thought) look under the same
\texttt{claim\_<id>/agent\_outputs/} folder where each agent writes an \texttt{output.json} plus helper metadata.
...
For newcomers, these are the high-leverage places to explore:
\begin{itemize}[leftmargin=*]
  ...
  \item \texttt{src/fact\_check/agents/} – implementation of the five specialised LLM agents.
