...
\draw[arrow] (image.south) -- ++(0,-3.5) -| (present);
...
\node[api, above=1cm of openai] (anthropic) {Anthropic API};
...
\draw[arrow] (ingestion.east) -- node[label, above] {Layout API} (gateway.west);
...
\draw[thick, dashed, gray] (-7.5,-3) rectangle (-1.5,2);
\draw[thick, dashed, gray] (-2.5,-3) rectangle (2.5,2);
\draw[thick, dashed, gray] (1.5,-3) rectangle (7.5,2);

\node at (-4.5,-2.7) [label, gray] {Docker};
