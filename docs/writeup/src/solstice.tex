...
After you run a fact-checking study (\texttt{python -m src.cli run-study}), the system creates a comprehensive directory structure containing all results:

\begin{verbatim}
data/studies/<study_name>/
|-- study_report.json              # High-level summary across all claims
|-- claim_1/
|   |-- evidence_report.json       # Final formatted evidence report
|   |-- agent_outputs/
|   |   |-- evidence_extractor/
|   |   |   |-- output.json        # Raw extracted quotes
|   |   |   `-- metadata.json      # Processing metadata
|   |   |-- completeness_checker/
|   |   |   |-- output.json        # Additional quotes found
|   |   |   `-- metadata.json      
|   |   |-- evidence_verifier_v2/
|   |   |   |-- output.json        # Verification results
|   |   |   `-- metadata.json      
|   |   |-- image_evidence_analyzer/
|   |   |   |-- output.json        # Visual evidence analysis
|   |   |   `-- metadata.json      
|   |   `-- evidence_presenter/
|   |       |-- output.json        # Final consolidated report
|   |       `-- metadata.json      
|   `-- logs/
|       `-- orchestration.log      # Detailed processing logs
|-- claim_2/
|   |-- evidence_report.json
|   |-- agent_outputs/
|   |   |-- ...                    # Same structure as claim_1
|   `-- logs/
`-- ...                            # Additional claims follow same pattern
\end{verbatim}

The \texttt{evidence\_report.json} file contains the final, formatted evidence ready for consumption by any UI or downstream system. Each agent's raw output is preserved in \texttt{agent\_outputs/} for debugging and analysis.
