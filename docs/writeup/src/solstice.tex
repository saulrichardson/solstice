...
\node[box, right=4cm of extract] (image) {Image Evidence\\Analyzer};
\node[box, below=of verify] (present) {Evidence\\Presenter};
\node[box, below=of present, fill=green!10] (result) {Consolidated\\Results};

% Scientific docs
\node[left=1.5cm of extract, align=center] (docs) {Scientific\\Documents};

% Arrows
\draw[arrow] (claim) -- (extract);
\draw[arrow] (extract) -- (complete);
\draw[arrow] (complete) -- (verify);
\draw[arrow] (verify) -- (present);
\draw[arrow] (present) -- (result);
\draw[arrow] (claim) -| (image);
\draw[arrow] (image) |- (present);
\draw[arrow, dashed] (docs) -- (extract);
\draw[arrow, dashed] (docs) |- (image);

% Labels
\node[right=0.1cm of extract, font=\footnotesize] {GPT-4.1};
\node[right=0.1cm of complete, font=\footnotesize] {GPT-4.1};
\node[right=0.1cm of verify, font=\footnotesize] {GPT-4.1};
\node[left=0.1cm of image, font=\footnotesize] {Vision};
...
Solstice is built on three core services that handle different aspects of the fact-checking pipeline:

\begin{figure}[h]
\centering
\begin{tikzpicture}[
    node distance=2cm,
    service/.style={rectangle, draw, rounded corners, minimum width=4cm, minimum height=2.5cm, align=center, fill=orange!10, thick},
    component/.style={rectangle, draw, minimum width=3cm, minimum height=0.6cm, align=center, fill=gray!10},
    api/.style={ellipse, draw, minimum width=2cm, minimum height=0.5cm, align=center, fill=yellow!10},
    arrow/.style={thick,->,>=stealth},
...
% Ingestion Service
\node[service, left=3cm of gateway] (ingestion) {
...
% Fact Check Service
\node[service, right=3cm of gateway] (factcheck) {
    \textbf{Fact Check Service}\\
    \vspace{0.3cm}
    Claim Processing\\
    Evidence Analysis\\
...
% External APIs
\node[api, above=1.5cm of gateway] (openai) {OpenAI\\API};
\node[api, above=0.5cm of openai] (anthropic) {Anthropic\\API};

% Data stores
\node[component, below=1.5cm of ingestion] (cache) {Document\\Cache};
\node[component, below=1.5cm of factcheck] (results) {Study\\Results};

% User entry points
\node[above=2.5cm of ingestion, label] (pdfin) {PDF Input};
\node[above=2.5cm of factcheck, label] (claims) {Claims Input};
...
\draw[arrow] (ingestion) -- node[label, above] {Layout API} (gateway);
\draw[arrow] (factcheck) -- node[label, above] {LLM Calls} (gateway);
\draw[arrow] (gateway) -- (openai);
\draw[arrow] (gateway) -- (anthropic);
\draw[arrow, dashed] (cache) -- (factcheck);

% Docker container boxes
\draw[thick, dashed, gray] (-6.5,-2.5) rectangle (-2,1);
\draw[thick, dashed, gray] (-2.2,-2.5) rectangle (2.2,1);
\draw[thick, dashed, gray] (2.5,-2.5) rectangle (6.5,1);

\node at (-4.25,-2.2) [label, gray] {Docker};
\node at (0,-2.2) [label, gray] {Docker};
\node at (4.5,-2.2) [label, gray] {Docker};

\end{tikzpicture}
\caption{Core Services Architecture: Three containerized services with Gateway as the central LLM interface}
\end{figure}
