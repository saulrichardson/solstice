...
% Labels
\node[right=0.2cm of extract, font=\small] {gpt-4.1};
\node[right=0.2cm of complete, font=\small] {gpt-4.1};
\node[right=0.2cm of verify, font=\small] {gpt-4.1};
\node[below=0.1cm of image, font=\small] {o4-mini};
...
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=0pt]
\item Evidence extraction from document text
\item Completeness checking to find missed quotes
\item Verification that quotes exist and support claims
\item Structured JSON output generation
\end{itemize}
...
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=0pt]
\item \textbf{Optimized prompts}: Text and image analysis require fundamentally different prompting strategies
\item \textbf{Parallel processing}: Text pipeline can complete while images are still being analyzed
\item \textbf{Resource efficiency}: Vision model calls are more expensive; separation allows selective processing
\item \textbf{Debugging clarity}: Failures in one modality don't affect the other
\end{itemize}
...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=0pt]
\item \textbf{Pipeline resilience}: If image analysis fails or times out, text evidence is still available
\item \textbf{Cost optimization}: Many claims can be verified from text alone; images are processed only when needed
\item \textbf{Specialized prompting}: Text extraction benefits from chain-of-thought reasoning, while image analysis needs spatial and visual reasoning
\item \textbf{Incremental development}: The text pipeline was built first and thoroughly tested before adding image support
\end{enumerate}
...
\item Parallel image processing is limited to 5 concurrent calls via semaphore
