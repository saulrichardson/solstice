"""PDF element catalog pipeline V4 - Improved text extraction with better spacing.

This pipeline:
1. Detects all elements (text, tables, figures)
2. Extracts text content for text elements with improved spacing
...
class PDFElementCatalogV4:
    """Creates a catalog of PDF elements with improved text extraction."""
    ...
    def _extract_text_content(
        ...
        """Extract text content for text elements with improved spacing."""
        results_with_text = []
                ...
                if page_num <= len(pdf.pages):
                    ...
                    for element in page_data['ordered_elements']:
                        ...
                        if element['type'] in ['text', 'title', 'list']:
                            ...
                            # Expand the bbox slightly to capture better word spacing
                            # This helps pdfplumber's layout analysis
                            margin = 2  # points
                            expanded_bbox = (
                                max(0, float(bbox[0]) - margin),  # x0
                                max(0, float(bbox[1]) - margin),  # y0 (bottom)
                                min(pdf_page.width, float(bbox[2]) + margin),  # x1
                                min(pdf_page.height, float(bbox[3]) + margin)   # y1 (top)
                            )
                            
                            try:
                                # Crop the page to the expanded bbox
                                cropped = pdf_page.within_bbox(expanded_bbox)
                                
                                # Extract text with layout preservation
                                # use_text_flow=True helps maintain proper spacing
                                text = cropped.extract_text(
                                    x_tolerance=3,  # Group characters within 3 points horizontally
                                    y_tolerance=3,  # Group lines within 3 points vertically
                                    layout=False,   # Don't try to preserve layout (we just want text)
                                    x_density=7.25, # Default density for character grouping
                                    y_density=13    # Default density for line grouping
                                )
                                ...
                                if text:
                                    ...
                                    # For multi-line text, ensure proper line breaks are preserved
                                    if '\n' in cleaned_text:
                                        # Preserve intentional line breaks but fix within-line spacing
                                        lines = cleaned_text.split('\n')
                                        fixed_lines = []
                                        for line in lines:
                                            # Check if line seems to have missing spaces (words run together)
                                            if self._needs_space_fixing(line):
                                                line = self._fix_spacing(line)
                                            fixed_lines.append(line)
                                        cleaned_text = '\n'.join(fixed_lines)
                                    else:
                                        # Single line - check if it needs space fixing
                                        if self._needs_space_fixing(cleaned_text):
                                            cleaned_text = self._fix_spacing(cleaned_text)
                                    
                                    element_copy['content'] = cleaned_text
                                    logger.debug(f"Extracted text from {element['type']} at {bbox}: {cleaned_text[:50]}...")
                                else:
                                    element_copy['content'] = ""
                                    logger.debug(f"No text found in {element['type']} at {bbox}")
    ...
    def _needs_space_fixing(self, text: str) -> bool:
        """Check if text seems to have missing spaces."""
        # Simple heuristic: look for lowercase followed by uppercase without space
        # or very long "words" that are likely multiple words joined
        import re
        
        # Pattern for lowercase letter followed immediately by uppercase
        if re.search(r'[a-z][A-Z]', text):
            return True
        
        # Check for very long "words" (likely multiple words joined)
        words = text.split()
        for word in words:
            # Skip URLs, file paths, etc.
            if '/' in word or '\\' in word or '.' in word:
                continue
            # If a "word" is suspiciously long, it might be multiple words
            if len(word) > 20 and word.isalpha():
                return True
        
        return False
    
    def _fix_spacing(self, text: str) -> str:
        """Attempt to fix spacing issues in text."""
        import re
        ...
        # e.g., "word123" -> "word 123" (but not for things like "H1N1")
    ...
    def _create_element_catalog(
        ...
        catalog = {
            "metadata": {
                ...
                "pipeline_version": "4.0-improved-text"
            },
            "pages": {},
            "elements": [],
            "statistics": {
...
def create_catalog_v4(
    pdf_path: str | Path,
    catalog_name: Optional[str] = None,
    output_dir: str = "output/catalogs"
) -> Path:
    """Convenience function to create a catalog with improved text extraction.
    ...
    """
    cataloger = PDFElementCatalogV4(output_base_dir=output_dir)
