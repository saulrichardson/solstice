"""PDF element catalog pipeline V5 - Full page text extraction with bbox matching.

This pipeline:
1. Detects all elements (text, tables, figures)
2. Extracts full page text with character positions
3. Maps text to detected bounding boxes
4. Keeps images/tables as references to be extracted on-demand
5. Creates an auditable catalog with separate stages
"""
...
class PDFElementCatalogV5:
    """Creates a catalog of PDF elements with full-page text extraction."""
    ...
    def create_catalog(
        ...
        try:
            ...
            results_with_text = self._extract_text_content_v5(pdf_path, ordered_results)
            if self.save_intermediate_stages:
    ...
    def _extract_text_content_v5(
        self,
        pdf_path: Path,
        ordered_results: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """Extract text using full page analysis then map to bounding boxes."""
        results_with_text = []
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for page_data in ordered_results:
                ...
                if page_num <= len(pdf.pages):
                    ...
                    # Extract full page text with character positions
                    # This gives us properly spaced text
                    ...
                    for element in page_data['ordered_elements']:
                        ...
                        if element['type'] in ['text', 'title', 'list']:
                            ...
                            # Find all characters within this bounding box
                            # Add small margin for better capture
                            margin = 1
                            chars_in_bbox = []
                            
                            for char in chars:
                                # Check if character center is within bbox
                                char_x = (char['x0'] + char['x1']) / 2
                                char_y = (char['y0'] + char['y1']) / 2
                                
                                if (bbox[0] - margin <= char_x <= bbox[2] + margin and
                                    bbox[1] - margin <= char_y <= bbox[3] + margin):
                                    chars_in_bbox.append(char)
                            
                            if chars_in_bbox:
                                # Sort characters by position (top to bottom, left to right)
                                chars_in_bbox.sort(key=lambda c: (-c['y0'], c['x0']))
                                ...
                                for char in chars_in_bbox:
                                    if current_y is None or abs(char['y0'] - current_y) <= line_tolerance:
                                ...
                                # Build text from characters with proper spacing
                                text_lines = []
                                for line in lines:
                                    # Sort characters in line left to right
                                    ...
                                    for char in line:
                                        if prev_char:
                                            ...
                                            space_width = char['x0'] - prev_char['x1']
                                            
                                            # Add space if gap is significant
                                            # Average character width as threshold
                                            avg_char_width = (prev_char['x1'] - prev_char['x0'] + 
                                                            char['x1'] - char['x0']) / 2
                                            
                                            if space_width > avg_char_width * 0.3:
                                                line_text += " "
                                ...
                                element_copy['content'] = extracted_text
                                
                                logger.debug(f"Extracted text from {element['type']} at {bbox}: {extracted_text[:50]}...")
                            else:
                                element_copy['content'] = ""
                                logger.debug(f"No characters found in {element['type']} at {bbox}")
    ...
    def _create_element_catalog(
        ...
        catalog = {
            "metadata": {
                ...
                "pipeline_version": "5.0-fullpage-extraction"
            },
            "pages": {},
            "elements": [],
            "statistics": {
...
def create_catalog_v5(
    pdf_path: str | Path,
    catalog_name: Optional[str] = None,
    output_dir: str = "output/catalogs"
) -> Path:
    """Convenience function to create a catalog with full-page text extraction.
    ...
    """
    cataloger = PDFElementCatalogV5(output_base_dir=output_dir)
