"""PDF element catalog pipeline V3 - Fixed coordinate conversion for text extraction.
                ...
                if page_num <= len(pdf.pages):
                    ...
                    for element in page_data['ordered_elements']:
                        ...
                        if element['type'] in ['text', 'title', 'list']:
                            ...
                            # pdfplumber uses same coordinate system as PDF (bottom-left origin)
                            # So we can use bbox directly: (x0, y0, x1, y1)
                            plumber_bbox = (
                                float(bbox[0]),  # x0
                            ...
                            try:
                                ...
                                if text:
                                    ...
                                    logger.debug(f"Extracted text from {element['type']} at {bbox}: {text[:50]}...")
                                else:
    ...
    def _create_element_catalog(
        ...
        catalog = {
            "metadata": {
                ...
                "pipeline_version": "3.0-fixed-coords"
            },
            "pages": {},
            "elements": [],
            "statistics": {
...
def create_catalog_v3(
    ...
    """
    cataloger = PDFElementCatalogV3(output_base_dir=output_dir)
