"""Simple test of reading order algorithm."""

# Define a simple Box class for testing
class Box:
    def __init__(self, id, bbox, page_index=0, label="Text"):
        self.id = id
        self.bbox = bbox
        self.page_index = page_index
        self.label = label

# Copy the algorithm here for testing
def determine_reading_order_simple(boxes, page_width=1600):
    """
    Simple reading order: left side first (top to bottom), then right side (top to bottom),
    with special handling for full-width elements at the bottom (e.g., footnotes).
    """
    if not boxes:
        return []
    
    # Estimate page height from boxes
    page_height = max(box.bbox[3] for box in boxes) if boxes else 3000
    
    # Find page midpoint
    midpoint = page_width / 2
    
    # Define a buffer zone around the midpoint to handle edge cases
    buffer = 50  # pixels
    
    # Identify full-width bottom elements (footnotes, page numbers, etc.)
    full_width_threshold = page_width * 0.75  # Elements spanning 75%+ of page width
    bottom_threshold = page_height * 0.8  # Elements in bottom 20% of page
    
    full_width_bottom = []
    regular_boxes = []
    
    for box in boxes:
        box_width = box.bbox[2] - box.bbox[0]
        box_top = box.bbox[1]
        
        # Check if this is a full-width element at the bottom
        if box_width >= full_width_threshold and box_top >= bottom_threshold:
            full_width_bottom.append(box)
        else:
            regular_boxes.append(box)
    
    # Process regular boxes with the column logic
    left_boxes = []
    right_boxes = []
    
    for box in regular_boxes:
        # Check if box is clearly in left column
        if box.bbox[0] < midpoint - buffer:
            left_boxes.append(box)
        # Check if box is clearly in right column
        elif box.bbox[0] > midpoint + buffer:
            right_boxes.append(box)
        else:
            # Box is in the buffer zone - decide based on its center
            box_center = (box.bbox[0] + box.bbox[2]) / 2
            if box_center < midpoint:
                left_boxes.append(box)
            else:
                right_boxes.append(box)
    
    # Sort each group by vertical position
    left_boxes.sort(key=lambda b: b.bbox[1])
    right_boxes.sort(key=lambda b: b.bbox[1])
    full_width_bottom.sort(key=lambda b: b.bbox[1])  # Sort footnotes by position too
    
    # Build reading order: left first, then right, then full-width bottom elements
    reading_order = []
    reading_order.extend([b.id for b in left_boxes])
    reading_order.extend([b.id for b in right_boxes])
    reading_order.extend([b.id for b in full_width_bottom])
    
    return reading_order


# Test with page 3 data
boxes = [
    Box("fab1ac32", [144.93, 1248.27, 1185.81, 3070.94]),  # Left column text
    Box("5fa0ac31", [384.33, 234.76, 2111.42, 1025.18]),   # Figure (spans columns)
    Box("5283ce44", [1257.37, 1241.07, 2288.50, 2515.88]), # Right column text #5
    Box("3b2440d7", [1253.57, 2681.99, 2292.54, 3066.76]), # Right column text #6
    Box("c5febf1e", [1240.04, 2593.24, 2300.42, 2678.68]), # Title (between 5 & 6)
    Box("a583e2de", [144.04, 1097.66, 2289.45, 1214.10]),  # Full-width text
]

page_width = 2481
reading_order = determine_reading_order_simple(boxes, page_width)

print("Reading order results:")
for i, box_id in enumerate(reading_order, 1):
    print(f"{i}. {box_id}")

# Check specific ordering
title_idx = reading_order.index("c5febf1e")
text5_idx = reading_order.index("5283ce44")
text6_idx = reading_order.index("3b2440d7")

print(f"\nPositions:")
print(f"Text #5 (5283ce44): position {text5_idx + 1}")
print(f"Title (c5febf1e): position {title_idx + 1}")
print(f"Text #6 (3b2440d7): position {text6_idx + 1}")

if text5_idx < title_idx < text6_idx:
    print("\n✓ SUCCESS: Title is correctly between text 5 and 6!")
else:
    print("\n✗ FAIL: Title ordering is still incorrect")